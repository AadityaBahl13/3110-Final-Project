...
let test_entropy_uniform_pos _ =
  ...
  assert_equal
    ~msg:"entropy on uniform positive should be 0"
...
let test_entropy_uniform_neg _ =
  ...
  let e = entropy data size (fun _ -> true) in
  assert_equal
    ~msg:"entropy on uniform negative should be 0"
    ~printer:string_of_float
    ~cmp:(fun a b -> abs_float (a -. b) < epsilon)
    0.0
    e
...
let test_entropy_balanced_default _ =
  ...
  let e = entropy data size (fun _ -> true) in
  assert_equal
    ~msg:"entropy on balanced with default split should equal ln 2"
    ~printer:string_of_float
    ~cmp:(fun a b -> abs_float (a -. b) < epsilon)
    ln2
    e
...
let test_entropy_find_split _ =
  ...
  let split_fn = find_split tree in
  let e = entropy data size split_fn in
  assert_bool
    "entropy for find_split on balanced should be ~0"
    (abs_float e < epsilon)
...
let test_predict_single _ =
  ...
  assert_equal
    ~msg:"predict should return original label"
    y
    lbl
...
let test_train_converges _ =
  ...
  List.iter
    (fun (x, lbl) ->
      assert_equal
        ~msg:(Printf.sprintf "classify %s"
                 (String.concat "," (List.map string_of_int (List.concat (to_list x)))))
        lbl
        (predict tree x))
    (data_to_list data)
  ...
  assert_equal
    ~msg:"split with max_splits=1 should not change root entropy"
    ~printer:string_of_float
    ~cmp:(fun a b -> abs_float (a -. b) < epsilon)
    orig_entropy
    new_entropy
